...
setwd(species.dir)
if (species.data$ANN==1){

if (round(gmin)==12) {source(paste(script.dir,'future.twelve_months.r', sep=''))} 
if (gmin<11.5) {source(paste(script.dir,'future.lessthan_twelve.r', sep=''))}
}
if (species.data$ANN==2) {source(paste(script.dir,'future.twelve_months.r', sep=''))} 
...
image(base.asc,ann=FALSE,axes=FALSE, col='gray90');
image(combine.asc, ann=FALSE,axes=FALSE, zlim=c(0,10), col=combine.cols, add=TRUE)
#text (130, -40, paste(species, ' ','Model', sep=''), cex=3)
legend.gradient(pnts, cols=cols,limits=c(0,10), title='Combined', cex=2.2)
