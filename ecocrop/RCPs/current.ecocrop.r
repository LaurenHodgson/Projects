...
} else {source(paste(script.dir,'current.lessthan_twelve.r', sep=''))}
} else {source(paste(script.dir,'current.twelve_months.r', sep=''))} 
...
png(paste(species.dir, '/', gsub(' ','_',species), '.', yr, '.png',sep=''), width=21, height=14.5, units='cm', res=300, pointsize=5, bg='white')
par(mfrow=c(2,3), mar=c(1,2,1,2),oma=c(1,1,7,1))
...
image(base.asc,ann=FALSE,axes=FALSE, col='gray90');
image(combine.asc, ann=FALSE,axes=FALSE, zlim=c(0,5), col='#003300', add=TRUE)
