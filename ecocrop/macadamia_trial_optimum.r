...
optemp.max = species.data$TOPMX
optemp.min = species.data$TOPMN
too.wet = species.data$RMAX
too.dry = species.data$RMIN
oprain.max = species.data$ROPMX
oprain.min = species.data$ROPMN
...
for (ii in 1:ncol(copy.temp)){
	copy.temp[which(copy.temp[,ii] > optemp.max),ii] = 1
	copy.temp[which(copy.temp[,ii] < optemp.min & copy.temp[,ii] >0),ii] = 1
	copy.temp[which(copy.temp[,ii] >=optemp.min & copy.temp[,ii] <=optemp.max),ii] = 2
...
for (ii in 1:ncol(copy.rain)){
	copy.rain[which(copy.rain[,ii] > oprain.max),ii] = 1
	copy.rain[which(copy.rain[,ii] < oprain.min & copy.rain[,ii] >0),ii] = 1
	copy.rain[which(copy.rain[,ii] >=oprain.min & copy.rain[,ii] <=oprain.max),ii] = 2
...
#copy.combine$total = rowSums(copy.combine[1:ncol(copy.combine)])  ### USE THIS TO ADD PH AND SAL
copy.combine$total = rowSums(copy.combine[1:2])
...
####################################07.make image
cols= c('gray90',colorRampPalette(c('red','wheat3'))(99),colorRampPalette(c('darkolivegreen3','darkgreen'))(99),'black')

phcols=colorRampPalette(c('red','yellow','forestgreen','blue','purple'))(100)

pnts=cbind(x=c(112,116,116,112), y=c(-11,-11,-18.5,-18.5))
png(paste(out.dir, '/', species, '.', 'gcm.variability.op.png',sep=''), width=24, height=14.5, units='cm', res=300, pointsize=5, bg='white')
par(mfrow=c(2,3), mar=c(0,2,0,0),oma=c(0,0,6,0))

image(temp.asc, ann=FALSE,axes=FALSE, col=cols)
#text (130, -40, 'Temperature', cex=3)
legend.gradient(pnts,cols=cols, limits=range(temp.asc,na.rm=TRUE), title='Temperature', cex=2.2)
