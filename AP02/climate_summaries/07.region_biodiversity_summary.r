...
for (type in types) {
	...
	for (r in regions){
		...
		for (es in ESs){
			...
			tout=matrix(NA,nc=12,nr=16)
			colnames(tout)=c('RCP','taxa','outcome', 'current',seq(2015,2085,10))
			tout=as.data.frame(tout)
			tout$RCP=es
			tout$taxa=c(taxa[1],taxa[1],taxa[1],taxa[1],taxa[2],taxa[2],taxa[2],taxa[2],taxa[3],taxa[3],taxa[3],taxa[3],'total','total','total','total')
			tout$outcome=c('kept','added','lost','total','kept','added','lost','total','kept','added','lost','total','kept','added','lost','total')
			for (tax in taxa){
