...
species.dir="/home/jc246980/ALA_downloads/Data/Inverts/Invert_species/"
species = list.files(species.dir)
species=gsub('.csv','',species)
species=species[11:length(species)]
...
for (spp in species){ cat(spp, '\n')
	...
	##create the sh file
	 zz = file(paste(spp,'.images.sh',sep=''),'w')
		 ...
		 cat("R CMD BATCH --no-save --no-restore '--args spp=\"",spp,"\" ' ~/scripts/NARP_freshwater/run_yabbies.r ",spp,'.Rout \n',sep='',file=zz) #run the R script in the background
	close(zz) 
