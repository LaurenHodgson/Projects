...
work.dir = "H:/Barra Work Directory/2080_GCMs/all/"; setwd(work.dir)
out.dir = "H:/Barra Work Directory/outputs/future_temp/"

folders = as.vector(list.files())
bccr_bcm2_0 = list.files(pattern='bccr_bcm2_0')
csiro_mk3_0 = list.files(pattern='csiro_mk3_0')
csiro_mk3_5 = list.files(pattern='csiro_mk3_5')
giss_aom = list.files(pattern='giss_aom')
inmcm3_0 = list.files(pattern='inmcm3_0')
miroc3_2_hires = list.files(pattern='miroc3_2_hires')
miroc3_2_medres = list.files(pattern='miroc3_2_medres')
ncar_ccsm3_0 = list.files(pattern='ncar_ccsm3_0')

###00.create mean.temp for each month in each 

sub.dir=NULL
for (aa in folders) {cat(aa,'\n')
		sub.dir=paste(work.dir, '/', aa, '/', sep='')
		setwd(sub.dir)
...
###01.create means of all gcms with multiple runs

gcm = c('bccr_bcm2_0', 'csiro_mk3_0', 'csiro_mk3_5', 'giss_aom', 'inmcm3_0', 'miroc3_2_hires', 'miroc3_2_medres', 'ncar_ccsm3_0') 
sum.runs=NULL
sub.dir=NULL
count.runs=c(1, 3, 3, 4, 1, 1, 9, 8)
for (aa in folders) {cat(aa,'\n')
		sub.dir=paste(work.dir, '/', aa, '/', sep='')
		setwd(sub.dir)

for (bb in gcm) {cat(bb,'\n')

for (ii in 1:12) { cat(ii,'\n')
		tasc.mean = read.asc.gz(paste("mean.temp", sprintf('%02i',ii), ".asc.gz",sep=''))
		
		if (length(sum.runs) == 0) { #if there are no values , populate it
          sum.runs = tasc.mean
        } else { #if already has values, add the new values to it
          sum.runs = sum.runs + tasc.mean
		}
	mean.runs = sum.runs/count.runs[bb]
		 
	write.asc.gz(mean.runs, paste(work.dir, gcm, ".", "mean.runs", sprintf('%02i',ii), ".asc",sep=''))
}	
}
}
