...
png(paste('barra_opt_range.annual.png',sep=''), width=7, height=7, units='cm', res=300, pointsize=5, bg='white')


		tasc.max = read.asc.gz("bioclim_05.asc.gz")
		tasc.min = read.asc.gz("bioclim_06.asc.gz")
		tasc.mean = read.asc.gz("bioclim_01.asc.gz")
	out = tasc.mean
	too.hot = which(tasc.max>400)
	too.cold = which(tasc.min<170)
