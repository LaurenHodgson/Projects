...
png(paste(out.dir, '/', 'barplots.only.other.png',sep=''), width=15, height=10, units='cm', res=300, pointsize=5, bg='white') #call image
...
for (jj in 1:length(weight.maps)) {  
      ...
      if (jj==1) {image(base.asc, ann=FALSE,axes=FALSE,col='gray86')
	  image(tasc, ann=FALSE,axes=FALSE,col=cols, zlim=c(1,7), add=TRUE)
      text (130, -40, 'Current', cex=3)} #text for current
	  ...
	  if (jj>1) {par(mar=c(4,6,5,2))
	  barplot(cs$total.area, main=paste(strsplit(ii, '\\.')[[1]][1],' ',strsplit(ii, '\\.')[[1]][2], sep=''), cex.main=3, ylim=c(0,2000), col=cols, axes=FALSE) #call barplot for 2030 - 2080 (min, mean, max) - note: 2030 might have fewer classes too
	  axis(2,at=seq(0,2000,500),labels=NA,lwd=1,lwd.ticks=1)}
	  
	  if (jj>1) {par(mar=c(1,2,2,1))
