...
tmin.dir = 'H:Barra Work Directory/AWAP.direct.download/tmin/daily/'
tmax.dir = 'H:Barra Work Directory/AWAP.direct.download/tmax/daily/'
out.dir= 'H:Barra Work Directory/outputs/daily_validation/'; dir.create(out.dir)

################################################################################
###set the variables
start.date = as.Date("2009-01-01"); end.date = as.Date("2010-12-31")
start.wt = 20
...
#out = read.csv(paste(out.dir, 'mrb_20090101-20101231', '.csv',sep=''))
out$length=NULL
out$length= (out$wt/0.0106)^(1/3.02)
#W = 0.0106*L^3.02

write.csv(out,file=paste(out.dir, 'mrb_20090101-20101231', '.csv',sep=''))

png(paste(out.dir, 'mareeba.end_weight.png',sep=''), width=21, height=14, units='cm', res=300, pointsize=5, bg='white')
par(mfrow=c(2,3), mar=c(5,7,4,4), oma=c(1,1,4,1))

	plot(out$julian.day,out$wt, main="Weight of Barramundi over time", xaxt='n', xlab=NA, ylab="Weight (g)", type='l', lty=3, cex.main=2.5, cex.lab=2.3, cex.axis=1.7)
		lines(out$julian.day[which(out$wt<=3000)],out$wt[which(out$wt<=3000)])
		axis(1,at=seq(14400,15000,200),labels=c('June 09',NA, 'June 2010',NA),lwd=1,lwd.ticks=1, cex.axis=1.7)
	plot(out$julian.day, out$gain, main="Weight gain of Barramundi over time", ylab="Weight (g)", xlab=NA,xaxt='n', type='l', lty=3, cex.main=2.5, cex.lab=2.3, cex.axis=1.7)
		lines(out$julian.day[which(out$wt<=3000)],out$gain[which(out$wt<=3000)])
		axis(1,at=seq(14400,15000,200),labels=c('June 09',NA, 'June 2010',NA),lwd=1,lwd.ticks=1, cex.axis=1.7)
	plot(out$julian.day,out$percent.gain, main="Time vs percent gain", ylab="% gain", xlab=NA,xaxt='n', type='l', lty=3, cex.main=2.5, cex.lab=2.3, cex.axis=1.7)
		lines(out$julian.day[which(out$wt<=3000)],out$percent.gain[which(out$wt<=3000)])
		axis(1,at=seq(14400,15000,200),labels=c('June 09',NA, 'June 2010',NA),lwd=1,lwd.ticks=1, cex.axis=1.7)
	plot(out$julian.day,out$Me.sum, main="Cumulative energy demand",ylab="kJ/d",xlab=NA,xaxt='n', type='l', lty=3, cex.main=2.5, cex.lab=2.3, cex.axis=1.7)
		lines(out$julian.day[which(out$wt<=3000)],out$Me.sum[which(out$wt<=3000)])
		axis(1,at=seq(14400,15000,200),labels=c('June 09',NA, 'June 2010',NA),lwd=1,lwd.ticks=1, cex.axis=1.7)
	plot(out$julian.day,out$Me, main="Maintenance energy demand", ylab="kJ/d", xlab=NA,xaxt='n', type='l', lty=3, cex.main=2.5, cex.lab=2.3, cex.axis=1.7)
		lines(out$julian.day[which(out$wt<=3000)],out$Me[which(out$wt<=3000)])
		axis(1,at=seq(14400,15000,200),labels=c('June 09',NA, 'June 2010',NA),lwd=1,lwd.ticks=1, cex.axis=1.7)
	plot(out$julian.day,out$length, main="Length over time", ylab="Length(cm)", xlab=NA,xaxt='n', type='l', lty=3, cex.main=2.5, cex.lab=2.3, cex.axis=1.7)
		lines(out$julian.day[which(out$wt<=3000)],out$length[which(out$wt<=3000)])
		axis(1,at=seq(14400,15000,200),labels=c('June 09',NA, 'June 2010',NA),lwd=1,lwd.ticks=1, cex.axis=1.7)
	title(main="Mareeba", outer=T,cex.main=3, font.main=3)
dev.off()
