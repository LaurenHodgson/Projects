...
start.date=as.Date("2008-04-01")
end.date=as.Date("2010-04-01")
...
save(out, file=paste(out.dir, 'jul.apr08.rData',sep=''))

#load(file=paste(out.dir, 'mrbaug_20080815-20100815.rData', sep=''))
...
day=c(14379, 14563, 14744) #julian date of 9month, 15 month and 21 month
mass=c(1200, 1500, 3000) #mass at mareeba at 9month, 15 month and 21 month

#image01 - wt over time
png(paste(out.dir, 'mrb.test.png',sep=''), width=7, height=7, units='cm', res=300, pointsize=5, bg='white')
par(mar=c(3,5,3,2))
	plot(out$julian.day,out$wt, ylim=c(0,8000), xlim=c(14106,14835), main="Weight of Barramundi over time",  xaxt='n', xlab=NA, ylab="Weight (g)", type='l', lty=3, cex.main=1.5, cex.lab=1.3, cex.axis=1.2)
		lines(out$julian.day[which(out$wt<=3000)],out$wt[which(out$wt<=3000)])
		axis(1,at=c(14106,14290,14471,14655, 14835),labels=c(NA,'Feb 2009',NA, 'Feb 2010', NA),lwd=1,lwd.ticks=1.2, cex.axis=1)
dev.off()
...
#image02 - wt over time compared with mareeba data	
png(paste(out.dir, 'mrb.stock_aug08.comparison.png',sep=''), width=7, height=7, units='cm', res=300, pointsize=5, bg='white')
par(mar=c(3,5,3,2))
	plot(out$julian.day,out$wt, ylim=c(0,8000), xlim=c(14106,14835), main="Weight of Barramundi over time",  xaxt='n', xlab=NA, ylab="Weight (g)", type='l', lty=3, cex.main=1.5, cex.lab=1.3, cex.axis=1.2)
		lines(out$julian.day[which(out$wt<=3000)],out$wt[which(out$wt<=3000)])
		axis(1,at=c(14106,14290,14471,14655, 14835),labels=c(NA,'Feb 2009',NA, 'Feb 2010', NA),lwd=1,lwd.ticks=1.2, cex.axis=1)
		points(day, mass, pch=8)
dev.off()

#image03 - wt gain over time mareeba
png(paste(out.dir, 'mrb.stock_aug08.gain.png',sep=''), width=7, height=7, units='cm', res=300, pointsize=5, bg='white')
par(mar=c(3,5,3,2))
	plot(out$julian.day, out$gain, ylim=c(0,20), xlim=c(14106,14835),main="Weight gain of Barramundi over time",  xaxt='n', xlab=NA, ylab="Weight (g)", type='l', lty=3, cex.main=1.5, cex.lab=1.3, cex.axis=1.2)
